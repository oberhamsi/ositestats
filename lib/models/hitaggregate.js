...
HitAggregate.create = function(dayOrMonth, site) {
   ...
   var result = store.query('select count(distinct ProcessedHit.unique) as uniques, count(ProcessedHit.id) as hits \
      from ProcessedHit where ProcessedHit.' + keyDayOrMonth + ' = :dayOrMonth and ProcessedHit.site = :site', {
   ...
   hitAggregate.uniques = result.uniques;
   hitAggregate.hits = result.hits;
