...
exports.index = {
   GET: function(req) {
      ...
      return Response.html(to_html(read(getResource('../assets/skins/dashboard.html')), {
...
exports.stats = function(req, siteKey, timeKey) {
   ...
   return Response.html(to_html(read(getResource('../assets/skins/stats.html')), {
      site: siteKey,
      duration: duration,
      timeKey: timeKey,
      ...
      humanDate: function() {
         var d = this['.'];
         return (parseInt(d.substring(4, 6), 10) + 1) + '/' + d.substring(0, 4)
      },

   }) + read(getResource('../assets/skins/templates.skin')));
};

exports.alldata = function(req, siteKey, timeKey) {
   var all = {
      'siteKey': siteKey,
      'timeKey': timeKey,
      'aggregatedata': aggregatedata(null, siteKey, timeKey),
      'aggregatedata/year': aggregatedata(null, siteKey, timeKey.substring(0,4)),
      'distributiondata/referer': distributiondata(null, siteKey, 'referer', timeKey),
      'distributiondata/userAgent': distributiondata(null, siteKey, 'userAgent', timeKey),
      'distributiondata/page': distributiondata(null, siteKey, 'page', timeKey),
      'distributiondata/age': distributiondata(null, siteKey, 'age', timeKey)
   };
 ...
 */
var aggregatedata = exports.aggregatedata = function (req, siteKey, timeKey) {
   var siteKey = siteKey || req.params.siteKey;
   var timeKey = timeKey || req.params.timeKey;
   var duration;
   ...
   var hitAggregates = store.query('from HitAggregate where HitAggregate.duration = :duration \
      and HitAggregate.' + duration +' = :timeKey and HitAggregate.site = :site order by HitAggregate.' + duration + ' desc', {
         timeKey: timeKey,
         site: site._id,
 ...
 */
var distributiondata = exports.distributiondata = function(req, siteKey, distributionKey, timeKey) {
   ...
   var distributions = store.query('from Distribution where Distribution.duration = "month" \
      and Distribution.key = :distributionKey and Distribution.month = :timeKey and Distribution.site = :site', {
   ...
   return Response.json({
      ...
      distributions: distributions.map(function(d) {
         // FIXME can't JSON.stringify() the entity
         return {
            distributions: d.distributions,
            distributionKey: d.distributionKey
         }
      }),
   });
}
